    ...
    try:
        ...
        if access_token:
            ...
            if type == "github":
                ...
                clone_url = urlunparse(
                    (parsed.scheme, f"oauth2:{access_token}@{parsed.netloc}", parsed.path, '', '', ''))
            elif type == "bitbucket":
                # Format: https://x-token-auth:{token}@bitbucket.org/owner/repo.git
                clone_url = urlunparse(
                    (parsed.scheme, f"x-token-auth:{access_token}@{parsed.netloc}", parsed.path, '', '', ''))
...
def read_all_documents(path: str, is_ollama_embedder: bool = None, excluded_dirs: List[str] = None, excluded_files: List[str] = None,
    ...
    if use_inclusion_mode:
        ...
        if "file_filters" in configs and "excluded_dirs" in configs["file_filters"]:
            final_excluded_dirs.update(
                configs["file_filters"]["excluded_dirs"])

        # Add any additional excluded files from config
        if "file_filters" in configs and "excluded_files" in configs["file_filters"]:
            final_excluded_files.update(
                configs["file_filters"]["excluded_files"])
...
def prepare_data_pipeline(is_ollama_embedder: bool = False):
    """
    ...
    # Determine if using Ollama embedder if not specified
    if is_ollama_embedder is None or False:
        is_ollama_embedder = check_ollama()
    ...
    #     )
    # Manoj: Use batch processing for all embedders including Ollama
    # Use batch processing for other embedders
    batch_size = embedder_config.get("batch_size", 500)
    embedder_transformer = ToEmbeddings(
        embedder=embedder, batch_size=batch_size
...
def transform_documents_and_save_to_db(
    ...
    if progress_callback:
        asyncio.create_task(progress_callback("Loading documents for embedding..."))

    db.load(documents)

    # Transform with progress reporting
    if progress_callback:
        asyncio.create_task(progress_callback("Generating embeddings..."))
    ...
    if progress_callback:
        asyncio.create_task(progress_callback(f"Successfully embedded {len(documents)} documents"))
    ...
    try:
        ...
        if "content" in content_data and "encoding" in content_data:
            if content_data["encoding"] == "base64":
                ...
                raise ValueError(
                    f"Unexpected encoding: {content_data['encoding']}")
        else:
    ...
    try:
        ...
        try:
            ...
            project_response = requests.get(
                project_info_url, headers=project_headers)
        ...
        if content.startswith("{") and '"message":' in content:
            try:
                ...
                if "message" in error_data:
                    raise ValueError(
                        f"GitLab API error: {error_data['message']}")
            except json.JSONDecodeError:
                pass
...
class DatabaseManager:
    ...
    def prepare_database(
        ...
        """
        Create a new database from the repository. (optionally report progress)
    ...
    def _create_repo(self, repo_url_or_path: str, repo_type: str = "github", access_token: str = None) -> None:
        ...
        try:
            ...
            save_db_file = os.path.join(
                root_path, "databases", f"{repo_name}.pkl")
    ...
    def prepare_db_index(
        self, is_ollama_embedder: bool = None,
        ...
        if self.repo_paths and os.path.exists(self.repo_paths["save_db_file"]):
            ...
            if progress_callback:
                asyncio.create_task(progress_callback("Loading cached embeddings..."))
        ...
        if progress_callback:
            asyncio.create_task(progress_callback("Reading repository files..."))
