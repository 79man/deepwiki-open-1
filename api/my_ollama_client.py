...
def extract_ollama_tool_calls(message: "Message") -> Optional[List[Function]]:
    ...
    tool_calls_data = message.get("tool_calls") if isinstance(
        message, dict) else getattr(message, "tool_calls", None)
...
def parse_generate_response(completion: "GenerateResponse") -> "GeneratorOutput":
    ...
    if "response" in completion:
        ...
        raw_response = completion.get("response", "")

        # Check for tool calls in the completion
        tool_calls = extract_ollama_tool_calls(completion) or []

        one_tool_call = tool_calls[0] if tool_calls else None

        return GeneratorOutput(
            data=raw_response if not one_tool_call else one_tool_call,
...
def parse_chat_messsage(completion: Dict[str, Any]) -> "GeneratorOutput":
    ...
    if "message" in completion:  # get both thinking and content
        ...
        tool_calls = extract_ollama_tool_calls(message) or []

        one_tool_call = tool_calls[0] if tool_calls else None

        return GeneratorOutput(
            # Return first tool call as data
            data=raw_response if not one_tool_call else one_tool_call,
            tool_use=one_tool_call,
            raw_response=raw_response,
            thinking=thinking,
...
class MyOllamaClient(ModelClient):
    ...
    **Streaming Support:**
    ...
    def __init__(self, host: Optional[str] = None):
        ...
        log.info("Manoj: MyOllamaClient::__init()")
    ...
    def parse_chat_completion(
        ...
        elif isinstance(completion, dict) and "response" in completion:
        ...
        if model_type == ModelType.EMBEDDER:
            ...
            if input is not None and input != "":
                ...
                if model_kwargs.get("generate", False):
                    final_model_kwargs["prompt"] = input
                    # <-- internal flag
                    final_model_kwargs["use_generate"] = True
                else:
